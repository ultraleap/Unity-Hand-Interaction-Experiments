...
/// <summary>
/// matt-f (TigerThrow): Copy of the CurvedUISettings class with some input stuff removed to fix duplication bug.
/// </summary>

namespace CurvedUI
{
    [AddComponentMenu("CurvedUI/CurvedUINoInput")]
    [RequireComponent(typeof(Canvas))]
    public class CurvedUINoInput : MonoBehaviour
    {
        ...
        //Global settings
        [SerializeField]
        CurvedUISettings.CurvedUIShape shape;
        ...
        public float GetCyllinderRadiusInCanvasSpace()
        {
            ...
            if (PreserveAspect)
            {
                if (shape == CurvedUISettings.CurvedUIShape.CYLINDER_VERTICAL)
        ...
        public Vector2 GetTesslationSize(bool modifiedByQuality = true)
        {
            ...
            if (Angle != 0 || (!PreserveAspect && vertAngle != 0))
            {
                switch (shape)
                {
                    case CurvedUISettings.CurvedUIShape.CYLINDER: ret /= GetSegmentsByAngle(angle); break;
                    case CurvedUISettings.CurvedUIShape.CYLINDER_VERTICAL: goto case CurvedUISettings.CurvedUIShape.CYLINDER;
                    case CurvedUISettings.CurvedUIShape.RING: goto case CurvedUISettings.CurvedUIShape.CYLINDER;
                    case CurvedUISettings.CurvedUIShape.SPHERE:
                        {
