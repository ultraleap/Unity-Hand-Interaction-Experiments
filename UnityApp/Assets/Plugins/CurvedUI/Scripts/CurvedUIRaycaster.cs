...
#region LIFECYCLE
        ...
        protected override void Start()
        {
            ...
            if (MainCamera.Instance.ActiveCam != null && MainCamera.Instance.ActiveCam.GetComponent<GvrPointerPhysicsRaycaster>() != null)
            {
                LayerMask mask = MainCamera.Instance.ActiveCam.GetComponent<GvrPointerPhysicsRaycaster>().eventMask;
                ...
                if (IsInLayerMask(this.gameObject.layer, mask)){
                    Debug.LogWarning("CURVEDUI: GvrPointerPhysicsRaycaster is raycasting over this canvas' layer (" +this.gameObject.name +" - " + LayerMask.LayerToName(this.gameObject.layer)+" layer). "
                        ...
                        + "Make sure your GvrPointerPhysicsRaycaster is not raycasting on this object's layer UI by editing its properties. Click here to highlight it.", MainCamera.Instance.ActiveCam.gameObject);
                    mask = mask & ~(1 << this.gameObject.layer);
                    MainCamera.Instance.ActiveCam.GetComponent<GvrPointerPhysicsRaycaster>().eventMask = mask;
                }
            }
#endif 
        }
...
#region GRAPHIC RAYCASTING
        ...
        void FlatRaycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
        {
            ...
            int displayIndex;
            var currentEventCamera = eventCamera; // Property can call MainCamera.Instance.ActiveCam, so cache the reference instead
