...
namespace Leap.Unity.InputModuleAurora
{
    ...
    public class LeapInputModule : BaseInputModule
    {
        ...
        [SerializeField] private Sprite EnabledPointerSprite;
        private Sprite DisabledPointerSprite;
        ...
        public bool isUsingEyeGazeInput = false;
        private Vector3 eyeGazePosition = new Vector3(0, 0, 0);
        private bool eyeGazeIsClicking = false;
        ...
        //Misc. Objects
        CurvedUI.CurvedUIRaycaster[] curvedPanels;
        ...
        public Vector3 sharedIndexFingerPos;
        public Vector2 sharedPointerPos;
        public float sharedDistFromCurve;
        ...
        protected override void Awake()
        {
            ...
            DisabledPointerSprite = PointerSprite;
            ...
            curvedPanels = GameObject.FindObjectsOfType<CurvedUI.CurvedUIRaycaster>();
        ...
        void Update()
        {
            ...
            //Hacky implementation of cursor in gaze mode
            if (isUsingEyeGazeInput && PointerSprite.name == "transparent")
            {
                PointerSprite = EnabledPointerSprite;
                for (int i = 0; i < Pointers.Length; i++) Pointers[i].GetComponent<SpriteRenderer>().sprite = PointerSprite;
                StandardColor.a = 1;
                TriggeringColor.a = 1;
                TriggerMissedColor.a = 1;
            }
            else if (!isUsingEyeGazeInput && PointerSprite.name != "transparent")
            {
                PointerSprite = DisabledPointerSprite;
                for (int i = 0; i < Pointers.Length; i++) Pointers[i].GetComponent<SpriteRenderer>().sprite = PointerSprite;
                StandardColor.a = 0;
                TriggeringColor.a = 0;
                TriggerMissedColor.a = 0;
            }

            if (Input.GetKeyDown(KeyCode.M)) eyeGazeIsClicking = true;
            if (Input.GetKeyDown(KeyCode.N)) isUsingEyeGazeInput = !isUsingEyeGazeInput;
        }
        ...
        public override void Process()
        {
            ...
            //For Tiger Throw: simulated input using eye gaze
            if (isUsingEyeGazeInput)
            {
                RaycastHit[] hits = Physics.RaycastAll(Camera.main.transform.position, Camera.main.transform.forward);
                foreach (RaycastHit hit in hits)
                {
                    if (hit.collider.gameObject.tag != "CurvedUICollider") continue;
                    eyeGazePosition = hit.point;
                    if (!eyeGazeIsClicking) eyeGazePosition -= Camera.main.transform.forward * 0.05f;
                    else
                    {
                        eyeGazeIsClicking = false;
                        eyeGazePosition += Camera.main.transform.forward * 0.01f;
                    }
                    break;
                }
            }
            ...
            for (int whichPointer = 0; whichPointer < NumberOfPointers; whichPointer++)
            {
                ...
                if (perFingerPointer)
                {
                    ...
                    if (!isUsingEyeGazeInput && LeapDataProvider.CurrentFrame.Hands.Count - 1 < whichHand)
                    {
                        ...
                        HandlePreviousEvents(whichPointer, whichHand, whichFinger, true);
                        continue;
                    }
                }
                else
                ...
                if (Camera.main != null && LeapDataProvider != null && LeapDataProvider.CurrentFrame != null && LeapDataProvider.CurrentFrame.Hands != null && LeapDataProvider.CurrentFrame.Hands.Count - 1 >= whichHand)
                {
                ...
                {
                    Vector3 IndexMetacarpal = (isUsingEyeGazeInput) ? eyeGazePosition : LeapDataProvider.CurrentFrame.Hands[whichHand].fingers[whichFinger].GetBone(Bone.BoneType.METACARPAL).Center;
                ...
                if ((PointEvents[whichPointer] != null))
                {
                    ...
                    {
                        prevOverGo[whichPointer] = currentOverGo[whichPointer];
                        currentOverGo[whichPointer] = null;
                    ...
                    HandlePreviousEvents(whichPointer, whichHand, whichFinger);
        ...
        private void HandlePreviousEvents(int whichPointer, int whichHand, int whichFinger, bool handDisabled = false)
        {
            if (PrevTriggeringInteraction[whichPointer] && (handDisabled || (!isTriggeringInteraction(whichPointer, whichHand, whichFinger)) || (pointerState[whichPointer] == pointerStates.OffCanvas)))
            {
        ...
        private bool GetLookPointerEventData(int whichPointer, int whichHand, int whichFinger, Vector3 Origin, Vector3 Direction, bool forceTipRaycast)
        {
            ...
            if (isUsingEyeGazeInput) IndexFingerPosition = eyeGazePosition;
            else
            {
                if (getTouchingMode(whichPointer) || forceTipRaycast)
                {
                    TipRaycast = true;
                    IndexFingerPosition = LeapDataProvider.CurrentFrame.Hands[whichHand].fingers[whichFinger].TipPosition;
                    //Else Raycast through the knuckle of the Index Finger
                }
                else
                {
                    Camera.main.transform.position = Origin;
                    IndexFingerPosition = LeapDataProvider.CurrentFrame.Hands[whichHand].fingers[whichFinger].GetBone(Bone.BoneType.METACARPAL).Center;
                }
            }
            ...
            //Remap pointer pos from curve to 2d panel
            Vector2 screenPos = Camera.main.WorldToScreenPoint(IndexFingerPosition);
            sharedPointerPos = screenPos;
            for (int i = 0; i < curvedPanels.Length; i++)
            {
                if (curvedPanels[i].RaycastToCyllinderCanvas(new Ray(Camera.main.transform.position, IndexFingerPosition - Camera.main.transform.position), out screenPos))
                {
                    sharedPointerPos = screenPos;
                    break;
                }
            }
            sharedIndexFingerPos = IndexFingerPosition;

            //Calculate dist between curve and panel
            Vector3 curvePos = Vector3.zero; Vector3 backplatePos = Vector3.zero;
            bool hitCurve = false; bool hitBackplate = false;
            RaycastHit[] hits = Physics.RaycastAll(Camera.main.transform.position, IndexFingerPosition - Camera.main.transform.position);
            foreach (RaycastHit hit in hits)
            {
                if (hit.collider.gameObject.tag == "CurvedUICollider")
                {
                    hitCurve = true;
                    curvePos = hit.point;
                }
                if (hit.collider.gameObject.tag == "CurvedUIBackplateCollider")
                {
                    hitBackplate = true;
                    backplatePos = hit.point;
                }
                if (hitCurve && hitBackplate) break;
            }

            //Set the Raycast Direction and Delta
            PointEvents[whichPointer].position = Vector2.Lerp(PrevScreenPosition[whichPointer], (hitCurve && hitBackplate) ? screenPos : (Vector2)Camera.main.WorldToScreenPoint(IndexFingerPosition), 1.0f);
            ...
            List<RaycastResult> bodge = new List<RaycastResult>();
            foreach (RaycastResult res in m_RaycastResultCache) if (res.gameObject != null && res.gameObject.GetComponent<RectTransform>()) bodge.Add(res);
            m_RaycastResultCache = bodge;
            ...
            sharedDistFromCurve = (hitBackplate && hitCurve) ? Vector3.Distance(backplatePos, curvePos) : 0.0f;
        ...
        private void ProcessStateEvents(int whichPointer)
        {
            if (TriggerHoverOnElementSwitch)
            {
                if ((PrevState[whichPointer] != pointerStates.OffCanvas) && (pointerState[whichPointer] != pointerStates.OffCanvas))
                {
                    if (currentOverGo[whichPointer] != prevOverGo[whichPointer])
                    {
                        ...
                        if (BeginHoverSound != null) SoundPlayer.PlayOneShot(BeginHoverSound);
            ...
            if (PrevState[whichPointer] == pointerStates.OnCanvas)
            {
                ...
                else if (pointerState[whichPointer] == pointerStates.PinchingToCanvas)
                {
                    ...
                    if (BeginMissedSound != null) SoundPlayer.PlayOneShot(BeginMissedSound);
                }
            }
            else if (PrevState[whichPointer] == pointerStates.PinchingToCanvas)
            {
                if (pointerState[whichPointer] == pointerStates.OnCanvas)
                {
                    ...
                    if (EndMissedSound != null) SoundPlayer.PlayOneShot(EndMissedSound);
                }
            }
            else if (PrevState[whichPointer] == pointerStates.OnElement)
            {
                if (pointerState[whichPointer] == pointerStates.OnCanvas)
                {
                    ...
                    if (EndHoverSound != null) SoundPlayer.PlayOneShot(EndHoverSound);
                }
                else if (pointerState[whichPointer] == pointerStates.PinchingToElement)
                {
                    //When you click on an element
                    if (BeginTriggerSound != null) SoundPlayer.PlayOneShot(BeginTriggerSound);
            ...
            else if (PrevState[whichPointer] == pointerStates.PinchingToElement)
            {
                ...
                else if (pointerState[whichPointer] == pointerStates.OnElement || pointerState[whichPointer] == pointerStates.OnCanvas)
                {
                    ...
                    if (EndTriggerSound != null) SoundPlayer.PlayOneShot(EndTriggerSound);
        ...
        void evaluatePointerSize(int whichPointer)
        {
            ...
            if (!isUsingEyeGazeInput && !perFingerPointer && !getTouchingMode(whichPointer))
            {
                if (whichPointer == 0)
                {
        ...
        public bool isTriggeringInteraction(int whichPointer, int whichHand, int whichFinger)
        {
            ...
            if (!isUsingEyeGazeInput && InteractionMode != InteractionCapability.Tactile)
            {
        ...
        public float distanceOfTipToPointer(int whichPointer, int whichHand, int whichFinger)
        {
            ...
            Vector3 TipPosition = (isUsingEyeGazeInput) ? eyeGazePosition : LeapDataProvider.CurrentFrame.Hands[whichHand].fingers[whichFinger].GetBone(Bone.BoneType.DISTAL).NextJoint;
            return (-Pointers[whichPointer].InverseTransformPoint(TipPosition).z * Pointers[whichPointer].lossyScale.z) - TactilePadding - sharedDistFromCurve;
        }
        ...
        public float distanceOfTipToElement(Transform UIElement, int whichHand, int whichFinger)
        {
            //Get Base of Index Finger Position
            Vector3 TipPosition = (isUsingEyeGazeInput) ? eyeGazePosition : LeapDataProvider.CurrentFrame.Hands[whichHand].fingers[whichFinger].GetBone(Bone.BoneType.DISTAL).NextJoint;
            return (-UIElement.InverseTransformPoint(TipPosition).z * UIElement.lossyScale.z) - TactilePadding - sharedDistFromCurve;
        }
        ...
        {
            if (isUsingEyeGazeInput) return true;
