...
namespace Leap.Unity.InputModuleAurora
{
    ...
    public class CompressibleUI : MonoBehaviour, ILeapWidget
    {
        ...
        public struct Layer
        {
            ...
            [Tooltip("Use app height mod (TIGER THROW)")]
            public bool UseAppHeight; //TT-49
            [Tooltip("Use toggle height mod (TIGER THROW)")]
            public bool UseToggleHeight; //TT-71
            ...
            public float BaseMaxFloatDistance
            {
                get
                {
                    if (UseAppHeight) return maxFloatDistanceApp;
                    else if (UseToggleHeight) return maxFloatDistanceToggle;
                    else if (maxFloatDistanceDefault != 0) return maxFloatDistanceDefault;
                    else
                    {
                        maxFloatDistanceDefault = MaxFloatDistance;
                        return maxFloatDistanceDefault;
                    }
                }
            }

            [Tooltip("The default base height above this element that the Layer will float")]
            public float maxFloatDistanceDefault;
            [Tooltip("The base height above this element that the Layer will float, if UseAppHeight is enabled")]
            public float maxFloatDistanceApp;
            [Tooltip("The base height above this element that the Layer will float, if UseToggleHeight is enabled")]
            public float maxFloatDistanceToggle;
        ...
        // If disabled, compressible UI floating state cannot change
        ...
        public void SetMaxDistance(float distGeneric, float distApp, float distToggle)
        {
            for (int i = 0; i < Layers.Length; i++)
            {
                Layers[i].maxFloatDistanceApp = distApp;
                Layers[i].maxFloatDistanceToggle = distToggle;
                Layers[i].maxFloatDistanceDefault = distGeneric;
            }
            TabPressed(HasBeenPressed, true);
        }
        ...
        public void DivideLayerHeightsOnToggle(Toggle toggle, float height = -1f)
        {
            if (toggle.isOn)
            {
                for (int i = 0; i < Layers.Length; i++)
                {
                    ...
                    Layers[i].MaxFloatDistance = height != -1 ? height / 2f : Layers[i].BaseMaxFloatDistance / 2;
                }
            }
            else
            {
                ...
                {
                    Layers[i].MinFloatDistance *= 2f;
                    Layers[i].MaxFloatDistance = height != -1 ? height : Layers[i].BaseMaxFloatDistance;
                }
            }
        }

        //MattF: Nabbing the above method to make a better one!
        public bool HasBeenPressed { get; private set; }
        public void TabPressed(bool isActive, bool force = false)
        {
            if (isActive == HasBeenPressed && !force) return;


            // Instead of dividing by two, use a base min/max float distance
            if (isActive)
            {
                for (int i = 0; i < Layers.Length; i++)
                {
                    Layers[i].MinFloatDistance /= 2f;
                    Layers[i].MaxFloatDistance = Layers[i].BaseMaxFloatDistance / 2;
                }
            }
            else
            {
                ...
                {
                    Layers[i].MinFloatDistance *= 2f;
                    Layers[i].MaxFloatDistance = Layers[i].BaseMaxFloatDistance;
                }
            }
            HasBeenPressed = isActive;
        }
